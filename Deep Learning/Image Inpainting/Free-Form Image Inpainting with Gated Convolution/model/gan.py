...
from .network import GatedConv2dWithActivation, GatedDeConv2dWithActivation, SNConvWithActivation, get_pad
import time
import math
import torchvision.utils as vutils


# The Self Attention is different from Context Attention.
# I don't know if is appropriate
class Self_Attn(nn.Module):
    ...
    def forward(self, x):
        ...
        # attention_show = attention[:, :, 2048].view(m_batchsize, 1, int(math.sqrt(attention.shape[1])),
        #                                             int(math.sqrt(attention.shape[1])))
        # vutils.save_image(attention_show,
        #                   '%s/attention.png' % 'output',
        #                   normalize=True)
...
class InpaintSAGNet(torch.nn.Module):
    """
    ...
    def __init__(self, n_in_channel=5):
        super(InpaintSAGNet, self).__init__()
        cnum = 32
