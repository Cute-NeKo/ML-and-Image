...
def gen_mask(flags):
    ...
    masks = np.ones((flags.batchsize, flags.img_size, flags.img_size), dtype=np.float32)
    ...
    if flags.mask_type == 'center':
        ...
        ratio = 0.8
        masks[np.random.random((flags.batchsize, flags.img_size, flags.img_size)) <= ratio] = 0.
    elif flags.mask_type == 'half':
        half_types = np.random.randint(4, size=flags.batchsize)
        masks = [half_mask(half_types[idx], flags.img_size) for idx in range(flags.batchsize)]
        masks = np.asarray(masks)
    elif flags.mask_type == 'pattern':
        masks = [pattern_mask(flags.img_size) for _ in range(flags.batchsize)]
        masks = np.asarray(masks)
    else:
