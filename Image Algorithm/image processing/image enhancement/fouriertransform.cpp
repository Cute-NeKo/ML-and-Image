...
int main22()
{
   Mat I = imread("h://hhh9.jpg", IMREAD_GRAYSCALE);       //读入图像灰度图
...
int main()
{
   Mat input=imread("h://city1.jpg", IMREAD_GRAYSCALE);

   imshow("input",input);
   int w=getOptimalDFTSize(input.cols);
   int h=getOptimalDFTSize(input.rows);
   cout<<w<<" "<<input.cols<<endl;
   cout<<h<<" "<<input.rows<<endl;
   Mat padded;
